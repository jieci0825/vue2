<template>
  <div class="cart">
    ...
    <div class="cart-main">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="shop in cartInfoList" :key="shop.id">
          <li class="cart-list-con1">
            <!-- 如果 isChecked 等于1就默认选中 -->
            <input
              type="checkbox"
              name="chk_list"
          ...
          <li class="cart-list-con5">
            ...
            <input
              ...
              minnum="1"
              class="itxt"
              @change="handler('change', $event.target.value * 1, shop)"
            />
            <a
              href="javascript:void(0)"
              class="plus"
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="deleteCartById(shop)">删除</a>
            <br />
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    // 删除购物车产品的数据
    async deleteCartById(shop) {
      try {
        await this.$store.dispatch('deleteCartListBySkuId', shop.skuId)
        this.getData()
      } catch (error) {
        alert(error.message)
      }
